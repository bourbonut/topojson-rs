...
mod parser {
        ...
        if let JsonValue::Object(obj) = value {
            ...
            let geometry = match r#type {
                "GeometryCollection" => GeometryType::GeometryCollection {
                    geometries: {
                        if let JsonValue::Array(geometries) =
                            value_from_key(obj, "geometries", value)?
                        {
                            geometries
                                .iter()
                ...
                "Point" => GeometryType::Point {
                    coordinates: array_from_item(
                        value_from_key(obj, "coordinates", value)?,
                        |item| item.as_f64(),
                    )
                ...
                "MultiPoint" => GeometryType::MultiPoint {
                    coordinates: array_from_item(
                        value_from_key(obj, "coordinates", value)?,
                        |item| array_from_item(item, |item| item.as_f64()),
                    )
                    .ok_or(format!(
    ...
    async fn test_parsing() -> Result<(), String> {
        ...
        let topology =
            TopoJSON::try_from(json::parse(&json_content).map_err(|e| {
                format!("Cannot parse the content through 'json': {}", e.to_string())
            })?)?;
